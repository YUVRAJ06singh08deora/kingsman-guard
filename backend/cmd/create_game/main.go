...
func createGameHandler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	// Initialize the service for creating a game
	...
	// Handle the POST request for creating a game
	if request.HTTPMethod == "POST" {
		// Call the handler to create a new game
		response, err = handlers.CreateGame(ctx, request, gameService)

		if err != nil {
			log.Printf("Error creating game: %v", err)
			return events.APIGatewayProxyResponse{
				StatusCode: 500,
				Body:       "Internal server error: failed to create game",
				Headers:    map[string]string{"Access-Control-Allow-Origin": "*"},
			}, nil
		}
	} else {
...
func main() {
	...
	lambda.Start(createGameHandler)
}
